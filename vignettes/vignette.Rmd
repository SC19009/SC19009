...
## Function_1:kde
The function kde conducts a kernel density estimation for single population in R. Package kedd is required to implement this function.  
The source R code for kde is as following:
```{r,eval=FALSE}
kde<-function(x){
  library(kedd)
  es<-dkde(x,deriv.order=0,h=h.amise(x,deriv.order=0)$h)
  hist(x,xlab="x",ylab="f(x)",main="Kernel density estimation",freq=F)
  lines(es,xlab="x",ylab="f(x)",col="blue",sub="")
}
```

To implement kde, input a vector composed of sample points. Corresponding output is a sample frequency histogram plotted the fitted density curve.  
...
## Function_2:jack
The function jack conducts a jackknife estimation in C++.  
...
To implement jack, input a vector composed of sample points. Corresponding output is a 2-dimension vector, whose first element is the mean estimate: $\hat\mu$ and the second element is the variance estimate for the estimated mean: $\hat{Var}(\hat\mu)$.  
